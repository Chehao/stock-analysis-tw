// https://github.com/cancerberoSgx/suitescript-types-tools-and-frameworks/blob/master/react-redux-client-app/src/components/data/react-bootstrap-table-next/types/types.d.ts
...
interface PaginationProps {
  ...
  */
  ...
  withFirstAndLast?: boolean /** always show the next and previous page button */;
...
interface FilterProps<Type extends TODO> {
  //TODO: is thihs the same as Filter?
...
interface SelectRowOptions {
  ...
  classes?: (row: TODO, rowIndex: number) => string | string;
  headerColumnStyle?: React.CSSProperties;
  selectColumnStyle?: React.CSSProperties;
  onSelect?: (row: TODO, isSelect: boolean, rowIndex: number, e: TODO) => void | boolean;
  onSelectAll?: (isSelect: boolean, rows: TODO[], e: TODO) => void | number[];
  nonSelectable?: number[];
  selectionRenderer?: ({ mode, checked, disabled }: { mode: string, checked: boolean, disabled: boolean }) => JSX.Element;
}
...
declare module 'react-bootstrap-table-next' {
  ...
  export default class BootstrapTable<FieldIds extends string> extends React.Component<
    BootstrapTableProps<FieldIds>,
    TODO
  > {}
...
```
    ...
    rowClasses?: (row: TODO, rowIndex: number) => string | string;
  }
   ...
   * */
  export interface Column<FieldId extends string = string> extends ColumnI<FieldId> {};
...
interface ColumnI<FieldId extends string = string> {
  /**
...
There's only one different for dummy column than normal column, which is dummy column will compare the whole row value instead of cell value when call shouldComponentUpdate. */
   ...
   */
  formatter?: (cell: any, row: any, rowIndex: number, formatExtraData: any) => function;
  csvFormatter?: (cell: any, row: any, rowIndex: number, formatExtraData: any) => function;
  headerStyle?: React.CSSProperties | ((colum, colIndex) => React.CSSProperties);
  style?: React.CSSProperties | ((cell: any, row: any, rowIndex: number, colIndex: number) => React.CSSProperties);
  headerTitle?: ((column, colIndex) => string) | boolean;
  filter?: TODO;
  title?: (cell: TODO, row: TODO, rowIndex: number, colIndex: number) => string | boolean;
  ...
  */
  sortFunc?<T>(a: T, b: T, order: 'asc' | 'desc', dataField: string, rowA: Row, rowB: Row): number;
  /**
  ...
  ```
  */
  sortCaret?: (order: SortOrder, column: TODO) => function;
...
declare module 'react-bootstrap-table2-toolkit' {
  ...
  export default class ToolkitProvider<FieldIds extends string> extends React.Component<
  toolkitProps<FieldIds>,
    TODO
  > {};

  export interface ToolkitChildrenProps<fieldIds extends string = string> {
    // It have the basic props from ToolkitProvider and also contain few internal data.
    baseProps: TODO;
  ...
  interface ToolkitProps<fieldIds extends string = string> {
    keyField: string;
    columns?: ColumnI<fieldIds>[];
    data: RowT<fieldIds>[];
    bootstrap4?: boolean;
    exportCSV?: boolean;
    search?: TODO;
    columnToggle?: TODO;
    children: (props: ToolkitChildrenProps) => JSX.Element;
  };
}